...
{
  // printf("Debug: mappages called with va = 0x%p, pa = 0x%p, size = %d, perm = %d\n", va, pa, size, perm);
  uint64 a, last;
  pte_t *pte;
...
void uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  uint64 a;
  pte_t *pte;
  // printf("Debug: uvmunmap called with va = 0x%p, size = %d, do_free = %d\n", va, npages, do_free);
  if ((va % PGSIZE) != 0)
    ...
    if ((*pte & PTE_V) && (*pte & PTE_U))
    {
      ...
      if ((*pte & PTE_COW))
      {
        ...
        else
        {
          ...
          {
            memmove(new_page, (char *)pa1, PGSIZE);
            kfree((char *)pa1);
            flags = PTE_FLAGS(*pte);
            flags = flags | PTE_W;
            flags = flags & (~PTE_COW);
